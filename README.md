...
- `/drafts` — показывает список черновиков
  
### Для администраторов
- `/pending` — список заявок на модерации
- `/history` — последние 20 обработанных заявок
- Под каждой заявкой — кнопки `✅ Одобрить` / `❌ Отклонить`/`✏️ Редактировать текст`
  (после нажатия «Отклонить» бот попросит причину)
- `/ban <user_id> [причина]` — добавить пользователя в черный список
- `/unban <user_id>` - удалить пользователя из черного списка
- `/blacklist` - список пользователей в черном списке 

## Замечания

- FSM хранится в Redis — состояния не сбрасываются при перезапуске бота.
